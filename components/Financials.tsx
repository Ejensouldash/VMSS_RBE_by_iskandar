    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
           ...
           <p className="text-sm text-slate-400">Real-time P&L analysis with AI Forecasting.</p>
        </div>
        <div className="flex gap-2">
           <button onClick={handleExport} className="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-emerald-950/20 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
         <div className="vm-glass p-5 rounded-xl">
            <p className="text-xs font-bold text-slate-400 uppercase">Total Revenue</p>
            <h3 className="text-2xl font-black text-indigo-400 mt-1">RM {pnlData.totalRevenue.toFixed(2)}</h3>
         </div>
         <div className="vm-glass p-5 rounded-xl">
            <p className="text-xs font-bold text-slate-400 uppercase">Cost (COGS)</p>
            <h3 className="text-2xl font-black text-rose-400 mt-1">RM {pnlData.totalCost.toFixed(2)}</h3>
         </div>
         <div className="vm-glass p-5 rounded-xl">
            <p className="text-xs font-bold text-slate-400 uppercase">Gross Profit</p>
            <h3 className="text-2xl font-black text-emerald-400 mt-1">RM {pnlData.grossProfit.toFixed(2)}</h3>
         </div>
         <div className="vm-glass p-5 rounded-xl">
            <p className="text-xs font-bold text-slate-400 uppercase">Net Margin</p>
            <h3 className="text-2xl font-black text-yellow-400 mt-1">{pnlData.margin.toFixed(1)}%</h3>
         </div>
      </div>

      {/* AI FORECAST CHART */}
      <div className="vm-glass p-6 rounded-xl relative overflow-hidden">
          <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-white">Monthly P&L Analysis (Last 12 Months + Forecast)</h3>
              <div className="flex items-center gap-2 bg-indigo-950/40 px-3 py-1 rounded-full border border-indigo-900/30">
                 <Sparkles size={14} className="text-indigo-400 animate-pulse" />
                 <span className="text-xs font-bold text-indigo-400">AI Predictive Model Active</span>
              </div>
          </div>
          ...
          <div className="mt-4 p-4 bg-indigo-950/20 rounded-lg border border-indigo-900/30 flex items-start gap-3">
             <TrendingUp className="text-indigo-400 flex-shrink-0 mt-0.5" size={18} />
             <div>
                 <h4 className="font-bold text-sm text-white">AI Forecast Insight:</h4>
