    ...
    try {
        ...
        if (rawDate) {
            if (typeof rawDate === 'number') {
                ...
                // Cuci string dari aksara pelik (Trim & Remove invisibles)
                ...
                if (dParts.length === 3) {
                    ...
                    // 🛡️ Auto-Correction: Kalau Bulan > 12, mungkin format US (MM/DD/YYYY)
                    if (p2 > 12 && p1 <= 12) {
                        const temp = p1; p1 = p2; p2 = temp; // Swap
                    }
                    // Kalau Hari > 12 dan Bulan <= 12, kekalkan format DD/MM
                    // Kalau dedua <= 12, assume DD/MM (Standard Malaysia)
        ...
        if (rawTime !== undefined && rawTime !== null) {
            ...
            const cleanTime = tStr.replace(/[APM\s]/g, ''); // Buang AM/PM/Space
        ...
        // --- 3. FINAL VALIDATION (PENYELAMAT) ---
        // Kita cuba bina ISO string
        const finalISO = `${YYYY}-${MM}-${DD}T${String(HH).padStart(2, '0')}:${String(MIN).padStart(2, '0')}:${String(SEC).padStart(2, '0')}`;
        
        // Test: Adakah ini tarikh yang sah?
        const testDate = new Date(finalISO);
        if (isNaN(testDate.getTime())) {
            console.warn("Invalid Date detected during merge. Fallback to NOW.");
            return new Date().toISOString(); // Fallback ke hari ini
        }
        ...
        return new Date().toISOString(); // Safety net terakhir
    }
  },
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1 font-medium flex items-center gap-2">
            ...
            Bulletproof Date Engine • Safety Fallback Active
          </p>
        </div>
          ...
          `}>
            ...
            {files.length > 0 ? (
              ...
              <div className="flex flex-col items-center">
                ...
                <p className="text-slate-700 font-bold text-lg">Upload Fail Excel/CSV</p>
                <p className="text-xs text-slate-400 mt-2 px-10">Sistem akan automatik membaiki tarikh yang rosak atau terbalik.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col">
          {isProcessing ? (
            <div className="flex-1 flex flex-col items-center justify-center bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <p className="font-bold text-slate-600">Memproses Data...</p>
            </div>
          ) : parsedRows.length > 0 ? (
              ...
              <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
                <div className="grid grid-cols-2 gap-6 relative z-10">
                  <div className="space-y-1">
                    <p className="text-xs text-emerald-100 font-bold">REKOD DIJUMPA</p>
                    <p className="text-4xl font-black">{importStats?.totalRows}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="text-xs text-emerald-100 font-bold">JUMLAH JUALAN</p>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
              ...
              <p className="text-center text-sm font-medium px-10">Upload fail untuk mulakan.</p>
            </div>
          )}
        </div>
      </div>
